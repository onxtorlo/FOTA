...
def suit_mfst(mfst, options):
    ...
    digest.update(cbor.dumps(mfst_tmp.value[SUITEnvelope.fields['manifest'].suit_key]))
    mfst_tmp.value[SUITEnvelope.fields['auth'].suit_key] = cbor.dumps([
    ...
    mfst_envelope_tagged = cbor.dumps(mfst_tmp, canonical=True)
    return mfst_envelope_tagged

def main(options):
    m = json.loads(options.input_file.read(), object_pairs_hook=OrderedDict) if options.input_file else {'components':[]}
    ...
    if m.get('severable') or (hasattr(options, 'severable') and options.severable):
        nm.suit_envelope = nm.suit_envelope.to_severable('sha256')
    output = {
