...
from suit_tool.manifest import SUITComponentId, SUITCommon, SUITSequence, \
                     ...
                     SUITDigest, SUITDependencies, SUITDependency, SUITEnvelopeTagged \
...
# TODO: Select param directive using capability reporting
def make_sequence(cid, choices, seq, params, cmds, pcid_key=None, param_drctv='directive-override-parameters'):
...
def compile_manifest(options, m):
    ...
    for c in m['components']:
        ...
        if 'vendor-domain' in c and 'vendor-id' not in c:
            c['vendor-id'] = uuid.uuid5(uuid.NAMESPACE_DNS, c['vendor-domain']).hex

        if 'model-name' in c and 'class-id' not in c and 'vendor-id' in c:
            c['class-id'] = uuid.uuid5(uuid.UUID(c['vendor-id']), c['model-name']).hex
    ...
    CommonCmds = {
        'slot': lambda cid, data: mkCommand(cid, 'condition-component-slot', None),
    ...
    CommonParams = {
        ...
        'slot' : lambda cid, data: ('slot', data['slot'])
    }
    # print('Common')
    ...
    if 'dependencies' in m:
        for dep in m['dependencies']:
            ...
            with open(dep['file'],'rb') as dep_fd:
                ...
                cmfst = dep_envelope.value[SUITEnvelope.fields['manifest'].suit_key]
    ...
    for cid, choices in cid_data.items():
        if any([c.get('install-on-download', True) and 'uri' in c for c in choices]):
            ...
            InstCmds = {
                'slot': lambda cid, data: mkCommand(
                    cid, 'condition-component-slot', None)
            }
            ...
            FetchCmds = {
                'slot': lambda cid, data: mkCommand(
                    cid, 'condition-component-slot', data['slot']),
    ...
    mtext = {}
    for k in ['manifest-description', 'update-description', 'software-bill-of-materials']:
        if k in m:
            mtext[k] = m[k]
    for c in m['components']:
        ctext = {}
    ...
    tagged_manifest = SUITEnvelopeTagged().from_json({"SUITEnvelope_Tagged":jenvelope})
    # wrapped_manifest = SUITEnvelope().from_json(jenvelope)

    # return wrapped_manifest
    return tagged_manifest
